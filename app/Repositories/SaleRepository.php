...
class SaleRepository extends BaseRepository
{
        ...
        try {
            ...
            foreach ($input['sale_items'] as $saleItem) {
                ...
                if ($saleItem['unit'] == 1) {
                    ...
                    if ($product && $product->quantity >= $saleItem['quantity'] * $saleItem['box_per_qty']) {

                        $totalQuantity = $product->quantity - $saleItem['quantity'] * $saleItem['box_per_qty'];
                        $product->update([
                            'quantity' => $totalQuantity,
                        ]);
                    } else {
    ...
    public function calculationSaleItems($saleItem)
    {
        ...
        if ($saleItem['unit'] == 1) {
            ...
            $saleqty =  $saleItem['quantity'] * $saleItem['box_per_qty'];
            $saleItem['sub_total'] = ((isset($saleItem['fix_net_unit']) ? $saleItem['fix_net_unit'] : $saleItem['net_unit_cost']) + $perItemTaxAmount) * $saleItem['quantity'];
        }
        ...
        try {
            ...
            foreach ($input['sale_items'] as $key => $saleItem) {
                ...
                $saleItemArray = Arr::only($saleItem, [
                    ...
                    'sub_total','unit','box_per_qty','box_per_price'
                ]);
                ...
                if (is_null($saleItem['sale_item_id'])) {
                    ...
                    $saleItemArray = Arr::only($saleItem, [
                        ...
                        'discount_type', 'discount_value', 'discount_amount', 'sale_unit', 'quantity', 'sub_total','unit','box_per_qty','box_per_price'
                    ]);
            ...
            if ($oldItem && $oldItem->quantity != $saleItem['quantity']) {
                ...
                if ($saleItem['unit'] == 1) {
                    if ($saleItem['unit'] == $oldItem->unit) {
                        ...
                        // dd('oki2');
                        if ($oldItem->quantity * $oldItem->box_per_qty > $saleItem['quantity']) {
                            if ($product) {
                                $totalQuantity = $product->quantity + ($oldItem->quantity * $oldItem->box_per_qty - $saleItem['quantity']);
                                $product->update([
                                    'quantity' => $totalQuantity,
                                ]);
                            } else {
                        ...
                        } elseif ($oldItem->quantity * $oldItem->box_per_qty < $saleItem['quantity']) {
                            $totalQuantity = $product->quantity - ($saleItem['quantity'] - $oldItem->quantity * $oldItem->box_per_qty);
                            if ($product->quantity < ($saleItem['quantity'] - $oldItem->quantity * $oldItem->box_per_qty)) {
                                throw new UnprocessableEntityHttpException('Quantity must be less than Available quantity.');
                            }
                            $product->update([
                                'quantity' => $totalQuantity,
                    ...
                    if ($saleItem['unit'] == $oldItem->unit) {
                        // dd("hello1");
                        if ($oldItem->quantity * $oldItem->box_per_qty > $saleItem['quantity'] * $saleItem['box_per_qty']) {
                            if ($product) {
                                $totalQuantity = $product->quantity + ($oldItem->quantity * $oldItem->box_per_qty - $saleItem['quantity']  * $saleItem['box_per_qty']);
                                $product->update([
                                    'quantity' => $totalQuantity,
                                ]);
                            } else {
                        ...
                        } elseif ($oldItem->quantity * $oldItem->box_per_qty < $saleItem['quantity']  * $saleItem['box_per_qty']) {
                            $totalQuantity = $product->quantity - ($saleItem['quantity']  * $saleItem['box_per_qty'] - $oldItem->quantity * $oldItem->box_per_qty);
                            if ($product->quantity < ($saleItem['quantity']  * $saleItem['box_per_qty'] - $oldItem->quantity * $oldItem->box_per_qty)) {
                                throw new UnprocessableEntityHttpException('Quantity must be less than Available quantity.');
                            }
                            $product->update([
                                'quantity' => $totalQuantity,
                        ...
                        // dd('hello2');
                        if ($oldItem->quantity > $saleItem['quantity'] * $saleItem['box_per_qty']) {
                            if ($product) {
                                $totalQuantity = $product->quantity + ($oldItem->quantity - $saleItem['quantity']  * $saleItem['box_per_qty']);
                                $product->update([
                                    'quantity' => $totalQuantity,
                                ]);
                            } else {
                        ...
                        } elseif ($oldItem->quantity < $saleItem['quantity']  * $saleItem['box_per_qty']) {
                            $totalQuantity = $product->quantity - ($saleItem['quantity']  * $saleItem['box_per_qty'] - $oldItem->quantity);
                            if ($product->quantity < ($saleItem['quantity']  * $saleItem['box_per_qty'] - $oldItem->quantity)) {
                                throw new UnprocessableEntityHttpException('Quantity must be less than Available quantity.');
                            }
                            $product->update([
                                'quantity' => $totalQuantity,
                ...
                if ($saleItem['unit'] != $oldItem->unit) {
                    if ($oldItem->unit == 1 && $saleItem['unit'] == 2) { 
                        // 🔹 Changing from pcs to box
                        // New quantity should be `saleItem['quantity'] * saleItem['box_per_qty']`
                        $convertedNewQty = $saleItem['quantity'] * $saleItem['box_per_qty'];
                        
                        // Stock adjustment
                        $totalQuantity = $product->quantity - ($convertedNewQty - $oldItem->quantity);
                    } 
                    elseif ($oldItem->unit == 2 && $saleItem['unit'] == 1) { 
                        // 🔹 Changing from box to pcs
                        // Convert the old quantity to pcs: `oldItem->quantity * oldItem->box_per_qty`
                        $convertedOldQty = $oldItem->quantity * $oldItem->box_per_qty;
                        
                        // Stock adjustment
                        $totalQuantity = $product->quantity + ( $convertedOldQty - $saleItem['quantity']);
                    }
