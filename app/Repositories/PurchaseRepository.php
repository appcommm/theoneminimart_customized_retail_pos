...
class PurchaseRepository extends BaseRepository
{
        ...
        try {
            ...
            foreach ($input['purchase_items'] as $purchaseItem) {
                if ($purchaseItem['unit'] == 1) {
                    ...
                    $qty = $purchaseItem['quantity']* $purchaseItem['box_per_qty'];
                    manageStock($input['warehouse_id'], $purchaseItem['product_id'], $qty);
                }
    ...
    public function calculationPurchaseItems($purchaseItem)
    {
        ...
        if ($purchaseItem['unit'] == 1) {
            ...
            $purchaseItem['sub_total'] = ($purchaseItem['box_per_purchase_cost'] + $perItemTaxAmount) * $purchaseItem['quantity'];
        }
        ...
        try {
            ...
            foreach ($input['purchase_items'] as $key => $purchaseItem) {
                ...
                $purchaseItemArr = Arr::only($purchaseItem, [
                    ...
                    'sub_total','unit','box_per_qty','box_per_price','box_per_purchase_cost'
                ]);
    ...
    public function updateItem($purchaseItem, $warehouseId): bool
    {
        try {
            ...
            $totalQuantity = 0;
            if ($product) {
                if ( $oldItem && $oldItem->quantity != $purchaseItem['quantity']) {
                    if ($purchaseItem['unit'] == 1) {
                        if ($purchaseItem['unit'] == $oldItem->unit) {
                            ...
                            if ($oldItem->quantity * $oldItem->box_per_qty > $purchaseItem['quantity']) {
                                $totalQuantity = $product->quantity - ($oldItem->quantity * $oldItem->box_per_qty - $purchaseItem['quantity']);
                            } elseif ($oldItem->quantity * $oldItem->box_per_qty < $purchaseItem['quantity']) {
                                $totalQuantity = $product->quantity + ($purchaseItem['quantity'] - $oldItem->quantity * $oldItem->box_per_qty);
                            }
                        }
                    }else{
                        if ($purchaseItem['unit'] == $oldItem->unit) {
                            if ($oldItem->quantity * $oldItem->box_per_qty > $purchaseItem['quantity'] * $purchaseItem['box_per_qty']) {
                                $totalQuantity = $product->quantity - ($oldItem->quantity * $oldItem->box_per_qty - $purchaseItem['quantity']  * $purchaseItem['box_per_qty']);
                            } elseif ($oldItem->quantity * $oldItem->box_per_qty < $purchaseItem['quantity'] * $purchaseItem['box_per_qty']) {
                                $totalQuantity = $product->quantity + ($purchaseItem['quantity']  * $purchaseItem['box_per_qty'] - $oldItem->quantity * $oldItem->box_per_qty);
                            }
                        }else{
                            if ($oldItem->quantity  > $purchaseItem['quantity']  * $purchaseItem['box_per_qty']) {
                                $totalQuantity = $product->quantity - ($oldItem->quantity - $purchaseItem['quantity'] * $purchaseItem['box_per_qty']);
                            } elseif ($oldItem->quantity < $purchaseItem['quantity'] * $purchaseItem['box_per_qty']) {
                                $totalQuantity = $product->quantity + ($purchaseItem['quantity'] * $purchaseItem['box_per_qty'] - $oldItem->quantity );
                            }
                        }
                    }
                    ...
                    if ($purchaseItem['unit'] != $oldItem->unit) {
                        if ($oldItem->unit == 1 && $purchaseItem['unit'] == 2) { 
                            $convertedNewQty = $purchaseItem['quantity'] * $purchaseItem['box_per_qty'];
                            
                            $totalQuantity = $product->quantity + ($convertedNewQty - $oldItem->quantity);
                        } 
                        elseif ($oldItem->unit == 2 && $purchaseItem['unit'] == 1) { 
                
                            $convertedOldQty = $oldItem->quantity * $oldItem->box_per_qty;
                            
                    
                            $totalQuantity = $product->quantity - ( $convertedOldQty - $purchaseItem['quantity']);
                        }
