...
class PurchaseAPIController extends AppBaseController
{
    ...
    public function pdfDownload(Purchase $purchase): JsonResponse
    {
        ...
        $pdfPath = 'pdf/Purchase-' . $purchase->reference_code . '.pdf';
        ...
        $html = View::make('pdf.purchase-pdf', compact('purchase', 'companyLogo'))->render();
        $mpdf->WriteHTML($html);
        Storage::disk(config('app.media_disc'))->put($pdfPath, $mpdf->Output('', 'S'));
        $data['purchase_pdf_url'] = Storage::url($pdfPath);
