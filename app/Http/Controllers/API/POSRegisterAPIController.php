...
class POSRegisterAPIController extends AppBaseController
{
    ...
    public function getRegisterDetails(Request $request)
    {
        ...
        $saleItems = SaleItem::selectRaw('product_id, SUM(quantity) as quantity, SUM(product_price) as product_price')
    ...
    public function getRegisterData($startDate, $endDate)
    {
        ...
        $data['today_sales_kpay_payment'] = SalesPayment::whereIn('sale_id', $saleIds)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('payment_type', SalesPayment::KPAY)
            ->sum('amount');
