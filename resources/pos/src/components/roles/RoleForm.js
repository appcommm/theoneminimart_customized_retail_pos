...
const RoleForm = (props) => {
    ...
    useEffect(() => {
        const permissionsArrays = permissions
            .filter((perm) => perm.selected === true)
            .map((rodeId) => rodeId.id);
    ...
    const disabled =
        saveButtonEnable.length === 0
            ...
            : singleRole &&
              ...
              JSON.stringify(singleRole.permissions.map((item) => item.id)) ===
    ...
    const handleValidation = () => {
        ...
        if (!rolesValue["name"]) {
            errorss["name"] = getFormattedMessage(
                "role.input.name.validate.label"
            );
        } else if (rolesValue["name"] && rolesValue["name"].length > 50) {
            errorss["name"] = getFormattedMessage(
                "role.input.name.valid.validate.label"
            );
    ...
    const handleChanged = (event) => {
        ...
        if (itemName === "all_check") {
            ...
            setAllChecked(!allChecked);
            setNewPer(
                permissions.map((item) => ({ ...item, selected: checked }))
            );
        } else if (excludedPermissions.includes(itemName)) {
            // Update the selected state of the clicked excluded permission
            setNewPer((prevPermissions) => {
                ...
                // Check if all excluded permissions are selected
                const allExcludedSelected = excludedPermissions.every(
                    (permissionName) => {
                ...
                // Auto-check 'Manage Reports' if all excluded permissions are selected
                return updatedPermissions.map((item) => {
                    if (item.name === "Manage Reports") {
                        return { ...item, selected: allExcludedSelected };
                    }
                    return item;
                });
            });
        } else if (itemName === "Manage Reports") {
            // Select or deselect all excluded permissions when 'Manage Reports' is toggled
            setNewPer((prevPermissions) => {
                const updatedPermissions = prevPermissions.map((item) => {
                    if (item.name === "Manage Reports") {
                        return { ...item, selected: checked };
                    }
                    if (excludedPermissions.includes(item.name)) {
                        return { ...item, selected: checked }; // Toggle all excluded permissions based on 'Manage Reports'
                    }
                    return item;
                });
            ...
            // Regular toggle for other permissions
            setNewPer(
