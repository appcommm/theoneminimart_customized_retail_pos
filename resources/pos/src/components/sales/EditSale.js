...
const EditSale = (props) => {
    ...
    const statusDefaultValue =
        sales.attributes &&
        sales.attributes.status &&
        statusFilterOptions.filter(
            (option) => option.id === sales.attributes.status
        );
    const selectedPayment =
        sales.attributes &&
        sales.attributes.payment_status &&
        paymentStatus.filter(
            (item) => item.value === sales.attributes.payment_status
        );
    const selectedPaymentType =
        sales.attributes &&
        sales.attributes.payment_type &&
        paymentType.filter(
            (item) => item.value === sales.attributes.payment_type
        );

    const itemsValue = sales &&
        sales.attributes && {
            ...
            sale_items: sales.attributes.sale_items.map((item) => ({
                ...
                short_name:
                    item.sale_unit &&
                    item.sale_unit.short_name &&
                    item.sale_unit.short_name,
                stock_alert: item.product && item.product.stock_alert,
                product_price: item.product_price,
                fix_net_unit:
                    item.unit == 1 ? item.product_price : item.box_per_price,
                net_unit_price: item.product_price,
                box_per_qty: item.box_per_qty,
                box_per_price: item.box_per_price,
                unit: item.unit,
                ...
                stock:
                    ...
                    item.product.stocks.filter(
                        (item) =>
                            item.warehouse_id === sales.attributes.warehouse_id
                    ),
                sub_total: item.sub_total,
                sale_unit:
                    item.sale_unit && item.sale_unit.id && item.sale_unit.id,
            ...
            payment_type: {
                label:
                    ...
                    selectedPaymentType[0].label,
                value:
                    selectedPaymentType &&
                    selectedPaymentType[0] &&
                    selectedPaymentType[0].value,
            },
            status_id: {
                label:
                    statusDefaultValue &&
                    statusDefaultValue[0] &&
                    statusDefaultValue[0].name,
                value:
                    statusDefaultValue &&
                    statusDefaultValue[0] &&
                    statusDefaultValue[0].id,
            },
        };
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle
                title={getFormattedMessage("sale.edit.title")}
                to="/app/sales"
            />
            {isLoading ? (
                <Spinner />
