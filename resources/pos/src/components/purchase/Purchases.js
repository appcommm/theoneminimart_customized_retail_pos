...
const Product = (props) => {
    ...
    const itemsValue =
        purchases.length > 0 && currencySymbol
            ? purchases.map((purchase) => {
                  const supplier = suppliers.find(
                      (supplier) =>
                          supplier.id === purchase.attributes.supplier_id
                  );
                  ...
                  return {
                      ...
                      due: 0,
                      payment: purchase.attributes.payment_type,
                      date: moment(purchase.attributes.date).format(
                          "YYYY-MM-DD"
                      ),
                      time: moment(purchase.attributes.created_at).format("LT"),
                      grand_total: purchase.attributes.grand_total,
                      paid_amount: purchase.attributes.paid_amount || 0, // Ensure paid_amount is handled
                      currency: currencySymbol,
                      id: purchase.id,
                  };
              })
    ...
    useEffect(() => {
        if (purchases.length > 0) {
            ...
            setTimeout(() => {
                const totals = calculateTotals(itemsValue);
                const totalRow = {
                    date: "",
                    time: "",
                    reference_code: "Total",
                ...
                // Update tableArray with fresh data
                setTableArray([...itemsValue, totalRow]);
            }, 0);
        } else {
            setTableArray([]); // Ensure table is cleared when no purchases exist
        }
    }, [purchases, currencySymbol]); // Add currencySymbol to dependencies
