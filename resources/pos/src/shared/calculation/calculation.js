...
export const subTotalCountPerBox = (cartItem) => {
    const totalAmount = taxAmountPerBox(cartItem) + amountBeforeTax(cartItem);
    return Number(+totalAmount * cartItem.quantity).toFixed(2);
};
...
export const discountAmountPerBox = (cartItem) => {
    if (cartItem.discount_type === "1" || cartItem.discount_type === 1) {
        return (+cartItem.box_per_price / 100) * +cartItem.discount_value;
    } else if (cartItem.discount_type === "2" || cartItem.discount_type === 2) {
        return +cartItem.discount_value;
    }
    return +cartItem.discount_amount.toFixed(2);
...
export const taxAmount = (cartItem) => {
    if (cartItem.tax_type === "2" || cartItem.tax_type === 2) {
        return (
            ((+cartItem.fix_net_unit - discountAmount(cartItem)) *
                +cartItem.tax_value) /
            (100 + +cartItem.tax_value)
        );
...
export const taxAmountPerBox = (cartItem) => {
    if (cartItem.tax_type === "2" || cartItem.tax_type === 2) {
        return (
            ((+cartItem.box_per_price - discountAmountPerBox(cartItem)) *
                +cartItem.tax_value) /
            (100 + +cartItem.tax_value)
        );
    } else if (cartItem.tax_type === "1" || cartItem.tax_type === 1) {
...
export const calculateCartTotalAmount = (carts, inputValue) => {
    ...
    let taxCal = (
        (totalAmountAfterDiscount * inputValue.tax_rate) /
        100
    ).toFixed(2);
    finalTotalAmount =
        Number(totalAmountAfterDiscount) +
        Number(taxCal) +
        Number(value.shipping);
